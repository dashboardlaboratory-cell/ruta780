...
try:
    np.linalg.solve(X_mal.T @ X_mal, X_mal.T @ y)
    print("solve con la matriz deficiente: no protesto")
except np.linalg.LinAlgError:
    print("solve con la matriz deficiente -> LinAlgError: matriz singular")
b = np.linalg.lstsq(X_mal, y, rcond=None)[0]
print("lstsq SI devuelve algo:", np.round(b, 4))
print("  y no protesta, aunque la solucion no sea unica: elige la de norma minima")
print("  el ajuste es el mismo por los dos caminos:",
      bool(np.allclose(X_mal @ b, X_bien @ np.linalg.lstsq(X_bien, y, rcond=None)[0])))
```

El punto que conviene retener es el del final. `solve` lanza `LinAlgError` y avisa; **`lstsq` devuelve cuatro números sin protestar**, y son los de la solución de norma mínima entre las infinitas que hay. El ajuste es correcto, las predicciones coinciden con las del modelo bien planteado, pero **los coeficientes no significan lo que parecen**, porque otra solución igual de válida daría otros valores. Un informe que los interprete uno a uno está informando de una elección arbitraria del algoritmo.
